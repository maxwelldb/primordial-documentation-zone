...
// This module will be embedded in the pre-installation assembly.
...
[IMPORTANT]
====
Retrieve the UUID of the Neutron network whose ports the poll mode drivers will be attached to. You must add this UUID as a value of `compute.platform.openstack.additionalNetworkIDs` in the `install-config.yaml` file before you deploy your cluster.
====
...
* Perform post-installation configuration for SR-IOV, including:
   ...
   ** Deploy a `testpmd` pod that uses OVS hardware offloading.
